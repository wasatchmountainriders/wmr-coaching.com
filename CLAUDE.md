...
- **Inside the girls' decks, never reference boys or men.** No "this skill is identical to the boys' program," no "watching a confident woman demo lands differently than a man," no "compared to boys' lap times," no "this is just for the boys" sample dialogue, no defensive "girls can absolutely learn this." All of those framings imply girls have a built-in comparison anxiety with men — and writing them into coach materials makes the program look sexist by treating the comparison as default. Just describe the skill. Just describe the rider. The coaches already understand the developmental nuance; spelling it out as gendered insults their judgment and reads as condescending to the riders.
- **In girls' decks, prefer "rider" over "girl" for descriptive use.** "Common in girls this age" → "common at this age." "A girl in the wrong gear" → "A rider in the wrong gear." Naming gender as the determinant of a coaching pattern implies it's gendered when it isn't. Exception: pronouns (she/her) are fine — they're already gendered by deck audience.
- **Female pro role models stand on their own.** Cite Puck Pieterse, Pauline Ferrand-Prévot, Haley Batten, Evie Richards as exemplars of the skill — not as compensation for male pros. Never "pro women manual just like pro men" or "watch a pro woman because…" — name them as riders, full stop.
- **PHV, periods, saddle fit, and the retention research stay.** These are biology and documented coaching reality, not gendered framing. Mention matter-of-factly without comparison to male development.
...
- **2026-05-14** — Header redesign across all 9 pages. Added `assets/logo.jpg` (WMR mountains-and-wordmark, JPEG with white background). Replaced text wordmark with the logo image (linked to home, `mix-blend-mode: multiply` so the white JPEG background blends into the cream paper). Made `.site-header` sticky (`position: sticky; top: 0; z-index: 100;`). Tightened padding from `1.25rem` to `0.75rem` so the taller logo doesn't bloat header height (now ~64px). Added a `@media (max-width: 640px)` rule that shrinks the logo to 32px on mobile.
- **2026-05-14** — Wired home-link on all 8 deck/reference pages. Wrapped "Wasatch Mountain Riders" in `<a href="index.html">` inside `.wordmark` on the 7 decks, added matching `.wordmark a { color: inherit; }` rule. Matches the existing precedent in `reference.html` (which already had this since day one) — relative `index.html` preserves offline (file://) browsing per the README's design intent. Added `/index.html → /` redirect to `vercel.json` so online clicks stay one redirect rather than chaining through cleanUrls. Curriculum content untouched.
